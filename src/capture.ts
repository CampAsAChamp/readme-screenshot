...
import { findFreePort, interpolatePort, runShellCommand, startServer, stopServer, waitForHealth } from "./server.js";
  ...
  await page.evaluate((targetSelector) => {
    ...
    return Promise.all(
      elements.flatMap((element) =>
        element
          ? Array.from(element.getAnimations()).map((animation) => animation.finished.catch(() => undefined))
          : [],
      ),
    );
  }, selector);
...
async function createContext(
  ...
  if (usesTheme) {
    await context.addInitScript(buildThemeInitScript(config.theme.storage_key!, config.theme.attribute!, themeMode));
  }
